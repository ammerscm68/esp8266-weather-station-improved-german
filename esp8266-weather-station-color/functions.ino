...
void drawCurrentWeatherDetail() {
  ...
  // String weatherIcon;
  // String weatherText;
  drawLabelValue(0, "Temperatur:", String(currentWeather.temp, 1) + (IS_METRIC ? " °C" : " °F"));
  drawLabelValue(1, "Wind:", String(currentWeather.windSpeed*3.6, 1) + (IS_METRIC ? " km/h" : " mph") );
  ...
  drawLabelValue(6, "Sicht:", String(currentWeather.visibility) + " m");
}
...
void drawForecastTable(uint8_t start) {
  ...
  for (uint8_t i = start; i < start + 4; i++) {
    ...
    gfx.setColor(MINI_BLUE);
    gfx.drawString(firstColumnLabelX, y + 15, "LF:");
    gfx.setColor(MINI_WHITE);
    gfx.drawString(firstColumnValueX, y + 15, "   " + String(forecasts[i].humidity) + " %");

    gfx.setColor(MINI_BLUE);
    gfx.drawString(firstColumnLabelX, y + 30, "RM: ");
    gfx.setColor(MINI_WHITE);
    gfx.drawString(firstColumnValueX, y + 30, "  " + String(forecasts[i].rain, 0) + (IS_METRIC ? " mm" : " in"));

    gfx.setColor(MINI_BLUE);
    gfx.drawString(secondColumnLabelX+1, y, "LD:");
    gfx.setColor(MINI_WHITE);
    gfx.drawString(secondColumnValueX, y, String(forecasts[i].pressure, 0) + " hPa");

    gfx.setColor(MINI_BLUE);
    gfx.drawString(secondColumnLabelX+1, y + 15, "WG:");
    gfx.setColor(MINI_WHITE);
    gfx.drawString(secondColumnValueX, y + 15, String(forecasts[i].windSpeed*3.6, 0) + (IS_METRIC ? " km/h" : " mph") );

    gfx.setColor(MINI_BLUE);
    gfx.drawString(secondColumnLabelX+1, y + 30, "WR: ");
    gfx.setColor(MINI_WHITE);
...
void drawAbout() {
  ...
  gfx.setFont(ArialRoundedMTBold_14);
  gfx.setTextAlignment(TEXT_ALIGN_CENTER);
  drawLabelValue(5, "Heap Mem:", String(ESP.getFreeHeap() / 1024) + " kb");
  drawLabelValue(6, "Flash Mem:", String(ESP.getFlashChipRealSize() / 1024 / 1024) + " MB");
  drawLabelValue(7, "WiFi Stärke:", String(WiFi.RSSI()) + " dB");
  drawLabelValue(8, "Chip ID:", String(ESP.getChipId()));
  drawLabelValue(9, "VCC: ", String(ESP.getVcc() / 1024.0) + " V");
  drawLabelValue(10, "CPU Freq.: ", String(ESP.getCpuFreqMHz()) + " MHz");
  char time_str[15];
  ...
  drawLabelValue(11, "Laufzeit: ", time_str);
  drawLabelValue(12, "IP-Adresse: ", WiFi.localIP().toString());
  gfx.setTextAlignment(TEXT_ALIGN_LEFT);
  gfx.setColor(MINI_YELLOW);
  gfx.drawString(13, 280, "Letzter Reset: ");
  gfx.setColor(MINI_WHITE);
